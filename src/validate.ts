...
import { describe, type RecordIndex } from './record.js';
...
export function validateRecordShape(
  ...
  const focus: Term = { termType: 'NamedNode', value: ix.record.focus };
  ...
  if (type.termType !== 'NamedNode' || type.value !== CONCEPT_DEFINITION) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `focus must be typed cdef:ConceptDefinition, got ${describe(type)}`,
    );
  }
  ...
  if (status.termType !== 'NamedNode' || !SEMANTIC_STATUSES.has(status.value)) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `cdef:semanticStatus must be one of the four status tokens, got ${describe(status)}`,
    );
  }
  ...
  if (isPrime) {
    for (const p of [PRIME_CATEGORY, CHART_EDITION, CHART_INDEX, EXPONENT]) allowedFocus.add(p);
  } else {
    allowedFocus.add(AXIOM);
    if (!isAxiomsOnly) allowedFocus.add(EXPLICATION);
    if (isMolecule) allowedFocus.add(GROUNDING_NOTE);
  }
  for (const p of focusPreds) {
    if (!allowedFocus.has(p)) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `predicate <${p}> is not allowed on a ${localName(status.value)} record's focus (closed shape)`,
      );
    }
  }
  ...
  if (isPrime) {
    ...
    for (const ax of axioms) validateAxiom(ix, ax, conceptRefs, allowSymbolic);
    ...
    if (isAxiomsOnly && axioms.length === 0) {
      throw new ConceptHashError(
        ERR.SHAPE,
        'an AxiomsOnly record must carry at least one cdef:axiom',
      );
    }
    if (isMolecule) {
      ...
      if (note.termType !== 'Literal' || note.datatype !== XSD_STRING || note.language !== '') {
        throw new ConceptHashError(
          ERR.SHAPE,
          'cdef:groundingNote must be a plain xsd:string literal',
        );
      }
      for (const ref of checkGroundingNote(note.value)) groundingRefs.add(ref);
    }
...
function assertReservedTokens(quads: Quad[]): void {
  for (const q of quads) {
    for (const t of [q.subject, q.predicate, q.object]) {
      if (t.termType !== 'NamedNode') continue;
      if (t.value.startsWith('urn:concept-def:')) {
        if (t.value === SELF) continue;
        if (!ALL_PROFILE_TERMS.has(t.value)) {
          throw new ConceptHashError(
            ERR.RESERVED_TOKEN,
            `<${t.value}> — the urn:concept-def: namespace (except #self and the profile vocabulary) is RESERVED in authored content (§5)`,
          );
        }
      }
    }
...
function assertStructuralBnodeCap(ix: RecordIndex): void {
  const structural = ix.record.bnodes.filter((b) => !ix.record.listCells.has(b)).length;
  if (structural > CAPS.maxStructuralBnodes) {
    throw new ConceptHashError(
      ERR.CAPS,
      `record has ${structural} structural blank nodes (cap ${CAPS.maxStructuralBnodes}, §5)`,
    );
  }
}
 ...
 */
export function checkConceptRef(t: Term, allowSymbolic: boolean, refs: Set<string>): void {
  if (t.termType !== 'NamedNode') {
    throw new ConceptHashError(
      ERR.REFERENCE_SYNTAX,
      `expected a concept reference, got ${describe(t)}`,
    );
  }
  const v = t.value;
  if (v === SELF) return;
  if (v.startsWith('urn:concept-def:')) {
    throw new ConceptHashError(
      ERR.RESERVED_TOKEN,
      `<${v}> is a reserved token, not a concept reference`,
    );
  }
  if (v.startsWith(MINT_SCHEME)) {
    if (!allowSymbolic) {
      throw new ConceptHashError(
        ERR.REFERENCE_SYNTAX,
        `symbolic reference <${v}> is not legal here`,
      );
    }
    refs.add(v);
    return;
...
function validatePrime(ix: RecordIndex, focus: Term): void {
  ...
  if (index.termType !== 'Literal' || index.datatype !== XSD_NON_NEG_INT) {
    throw new ConceptHashError(
      ERR.SHAPE,
      'cdef:chartIndex must be an xsd:nonNegativeInteger literal',
    );
  }
  const exponents = ix.readList(exponentHead).map((t) => {
    if (t.termType !== 'Literal' || t.datatype !== XSD_STRING || t.language !== '') {
      throw new ConceptHashError(
        ERR.SHAPE,
        `exponent list items must be plain xsd:string literals, got ${describe(t)}`,
      );
    }
    return t.value;
  });
  ...
  const mismatch: (why: string) => never = (why) => {
    throw new ConceptHashError(
      ERR.PRIME_LEXICON_MISMATCH,
      `prime record does not byte-match the pinned lexicon: ${why}`,
    );
  };
  ...
  if (entry === undefined) mismatch(`no prime with chartIndex ${index.value}`);
  if (category.value !== cdef(entry.category)) {
    mismatch(`category ${describe(category)} ≠ cdef:${entry.category} for ${entry.name}`);
  }
  if (
    exponents.length !== entry.exponents.length ||
    exponents.some((x, i) => x !== entry.exponents[i])
  ) {
    mismatch(
      `exponents [${exponents.join(', ')}] ≠ pinned [${entry.exponents.join(', ')}] for ${entry.name} (order is chart order)`,
    );
  }
}
...
function validateAxiom(
  ...
  if (node.termType !== 'BlankNode') {
    throw new ConceptHashError(
      ERR.SHAPE,
      `cdef:axiom must point at a blank node, got ${describe(node)}`,
    );
  }
  ...
  if (rel.termType !== 'NamedNode' || !AXIOM_RELATIONS.has(rel.value)) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `cdef:rel must be one of the closed axiom relations, got ${describe(rel)}`,
    );
  }
  if (rel.value === REL_PROPERTY_KIND) {
    if (target.termType !== 'NamedNode' || !PROPERTY_KIND_TOKENS.has(target.value)) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `cdef:propertyKind must target cdef:DatatypeProperty or cdef:ObjectProperty`,
      );
    }
    return;
  }
  if (rel.value === REL_BRIDGES_TO) {
    ...
    if (target.termType !== 'NamedNode' || !/^https?:\/\/[^\s<>"{}|\\^`]+$/.test(target.value)) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `cdef:bridgesTo must target an absolute http(s) IRI, got ${describe(target)}`,
      );
    }
    return;
  }
...
function validateRestriction(
  ...
  if (node.termType !== 'BlankNode') {
    throw new ConceptHashError(
      ERR.SHAPE,
      `a restriction target must be a blank node, got ${describe(node)}`,
    );
  }
  ...
  for (const p of RESTRICTION_CARDINALITY_PREDICATES) {
    ...
    if (v.termType !== 'Literal' || v.datatype !== XSD_NON_NEG_INT) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `restriction cardinality <${p}> must be xsd:nonNegativeInteger`,
      );
    }
  }
  let values = 0;
  ...
  if (values > 1) {
    throw new ConceptHashError(
      ERR.SHAPE,
      'a restriction may carry at most one of allValuesFrom/someValuesFrom',
    );
  }
  if (constraints === 0 && values === 0) {
    throw new ConceptHashError(
      ERR.SHAPE,
      'a restriction must carry at least one cardinality or value constraint',
    );
  }
}

// --- explication structural shape (§4.6/§4.7) ----------------------------------------------

interface ExplicationCounters {
  clauses: number;
}

function validateExplicationShape(
  ix: RecordIndex,
  node: Term,
  refs: Set<string>,
  allowSymbolic: boolean,
  ...
  if (frame.termType !== 'NamedNode' || !FRAMES.has(frame.value)) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `cdef:frame must be one of the three typed frames, got ${describe(frame)}`,
    );
  }
  ...
  if (referentsHead !== undefined) {
    ...
    if (decls.length > CAPS.maxReferents) {
      throw new ConceptHashError(
        ERR.CAPS,
        `explication exceeds ${CAPS.maxReferents} referents (cap, §5)`,
      );
    }
    for (const d of decls) validateReferentDecl(ix, d);
  }
  const counters: ExplicationCounters = { clauses: 0 };
  const clauses = ix.readList(ix.one(node, CLAUSES));
  if (clauses.length === 0) {
    throw new ConceptHashError(ERR.SHAPE, 'cdef:clauses must be a non-empty list');
  }
  for (const c of clauses) validateClause(ix, c, refs, allowSymbolic, counters);
}

function validateReferentDecl(ix: RecordIndex, node: Term): void {
  if (node.termType !== 'BlankNode') {
  ...
  if (kind.termType !== 'NamedNode' || !REF_KINDS.has(kind.value)) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `cdef:refKind must be one of the five referent kinds, got ${describe(kind)}`,
    );
  }
}
...
function isClauseNode(ix: RecordIndex, node: Term): boolean {
  if (node.termType !== 'BlankNode') return false;
...
function validateClause(
  ...
  refs: Set<string>,
  allowSymbolic: boolean,
  counters: ExplicationCounters,
): void {
  counters.clauses += 1;
  if (counters.clauses > CAPS.maxClauses) {
    throw new ConceptHashError(
      ERR.CAPS,
      `explication exceeds ${CAPS.maxClauses} clauses incl. quote clauses (cap, §5)`,
    );
  }
  if (node.termType !== 'BlankNode') {
...
function validateSlot(
  ...
  if (node.termType !== 'BlankNode') {
    throw new ConceptHashError(
      ERR.SHAPE,
      `a slot/adjunct must be a blank node, got ${describe(node)}`,
    );
  }
  ...
  if (role.termType !== 'NamedNode' || !roleSet.has(role.value)) {
    throw new ConceptHashError(
      ERR.SHAPE,
      `role ${describe(role)} is not in the closed role inventory (§4.4)`,
    );
  }
...
function validateFiller(
  ...
  if (node.termType !== 'BlankNode') {
    throw new ConceptHashError(
      ERR.SHAPE,
      `a filler must be an IRI or blank node, got ${describe(node)}`,
    );
  }
  const preds = ix.predicatesOf(node);
...
function validateSubstantivePhrase(
  ix: RecordIndex,
  node: Term,
  refs: Set<string>,
  allowSymbolic: boolean,
  ...
  if (det !== undefined) checkConceptRef(det, allowSymbolic, refs);
  const quant = ix.optional(node, QUANT);
  if (quant !== undefined) checkConceptRef(quant, allowSymbolic, refs);
  for (const m of ix.objects(node, MOD)) checkConceptRef(m, allowSymbolic, refs);
...
function assertClosed(
  ...
  for (const p of ix.predicatesOf(node)) {
    if (!allowed.has(p)) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `predicate <${p}> is not allowed on a ${kind} node (closed shape)`,
      );
    }
  }
}
...
export function assertLiteralWhitelist(quads: Quad[]): void {
  ...
  for (const q of quads) {
    ...
    if (q.predicate.value === RDF_FIRST) continue;
    if (!LITERAL_PREDICATES.has(q.predicate.value)) {
      throw new ConceptHashError(
        ERR.SHAPE,
        `literal ${describe(q.object)} in non-whitelisted position <${q.predicate.value}> (§5 literal whitelist)`,
      );
    }
  }
}
