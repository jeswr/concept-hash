...
import type { Term } from './quads.js';
...
interface Scope {
  /** Indices introduced and usable at the current point. */
  introduced: Set<number>;
  /** Parent scope (a quote inherits the outer scope's referents). */
  parent?: Scope;
}

function inScope(scope: Scope, index: number): boolean {
  let s: Scope | undefined = scope;
  while (s !== undefined) {
    if (s.introduced.has(index)) return true;
    s = s.parent;
  }
  return false;
}
...
interface Ctx {
  ...
  /** Declared referent kinds (from cdef:referents). */
  declaredKinds: Map<number, string>;
  /** Top-level bind indices seen (each declared non-implicit referent needs exactly one). */
  topLevelBinds: Map<number, number>;
}
 ...
 */
export function checkExplicationGrammar(ix: RecordIndex, registry: ConceptRegistry): void {
  ...
  if (explication === undefined) return;
    ...
    for (let i = 1; i <= ctx.declaredKinds.size; i++) {
      if (!ctx.declaredKinds.has(i)) {
        throw new ConceptHashError(
          ERR.GRAMMAR,
          `referent indices must be dense from 1 — missing ${i} (§4.2)`,
        );
      }
    }
  }

  const topScope: Scope = { introduced: new Set(frameImplicit) };
  ...
  for (const [idx] of ctx.declaredKinds) {
    if (frameImplicit.has(idx)) continue;
    const binds = ctx.topLevelBinds.get(idx) ?? 0;
    if (binds !== 1) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `declared referent ${idx} must have exactly one introducing cdef:bind occurrence, found ${binds} (§4.2)`,
      );
    }
  }
}

function readIndex(t: Term): number {
  ...
  if (!Number.isInteger(n) || n < 1) {
    throw new ConceptHashError(
      ERR.GRAMMAR,
      `referent indices are positive integers, got ${t.value}`,
    );
  }
  return n;
}
 ...
 */
function walkClauseList(ctx: Ctx, clauses: Term[], scope: Scope, insideQuote: boolean): void {
  for (const clause of clauses) {
    ...
    for (const idx of binds) {
      ...
      if (inScope(scope, idx)) {
        throw new ConceptHashError(
          ERR.GRAMMAR,
          `referent ${idx} bound while already in scope (§4.2)`,
        );
      }
      if (!insideQuote && !ctx.declaredKinds.has(idx)) {
        throw new ConceptHashError(
          ERR.GRAMMAR,
          `top-level bind of undeclared referent ${idx} — only quote-local referents may be bind-introduced without declaration (§4.2)`,
        );
      }
      if (insideQuote && ctx.declaredKinds.has(idx)) {
        throw new ConceptHashError(
          ERR.GRAMMAR,
          `quote-local bind reuses declared referent index ${idx} (§4.2)`,
        );
      }
...
function collectBinds(ctx: Ctx, node: Term, out: number[]): void {
  ...
  for (const q of ctx.ix.quadsOf(node)) {
    if (q.predicate.value === BIND) {
      ...
      if (!preds.has(QUOTE_CLAUSES)) collectBinds(ctx, q.object, out);
    }
  }
}

function primeName(ctx: Ctx, t: Term): string | undefined {
  return t.termType === 'NamedNode' ? ctx.registry.primeNameOf(t.value) : undefined;
}

type ClausePosition = 'top' | 'filler';

function checkClause(ctx: Ctx, node: Term, scope: Scope, position: ClausePosition): void {
  const { ix } = ctx;
  ...
  if (name === undefined || !(name in OPERATORS)) {
    throw new ConceptHashError(
      ERR.GRAMMAR,
      `cdef:op ${describe(op)} does not resolve to a prime in the closed operator inventory (§4.5)`,
    );
  }
  const spec = OPERATORS[name];
  if (spec === undefined) throw new ConceptHashError(ERR.GRAMMAR, 'unreachable');
  const present = new Set(
    [SCOPE, ANCHOR, ANTECEDENT, CONSEQUENT, CAUSE, EFFECT].filter(
      (p) => ix.optional(node, p) !== undefined,
    ),
  );
  ...
  if ((name === 'AFTER' || name === 'BEFORE') && !present.has(SCOPE) && position !== 'filler') {
    throw new ConceptHashError(
      ERR.GRAMMAR,
      `${name} without a scope is only legal as a time-adjunct filler (§4.5)`,
    );
  }
...
function checkPredClause(ctx: Ctx, node: Term, pred: Term, scope: Scope): void {
  const { ix } = ctx;
  const name = primeName(ctx, pred);
  if (name === undefined || !(name in VALENCY)) {
    throw new ConceptHashError(
      ERR.GRAMMAR,
      `cdef:pred ${describe(pred)} does not resolve to a predicate prime (§4.4)`,
    );
  }
  const frame = VALENCY[name];
  if (frame === undefined) throw new ConceptHashError(ERR.GRAMMAR, 'unreachable');
  const roles = new Map<string, Term>();
  ...
  if (name === 'TRUE') {
    ...
    if (u !== undefined) checkTrueUndergoer(ctx, u);
  }
...
function checkTrueUndergoer(ctx: Ctx, filler: Term): void {
  if (filler.termType === 'BlankNode') {
    const preds = ctx.ix.predicatesOf(filler);
    if (preds.has(QUOTE_CLAUSES)) return;
    const ref = ctx.ix.optional(filler, REF);
    if (ref !== undefined) {
      const idx = readIndex(ref);
      const kind = ctx.declaredKinds.get(idx);
      if (kind === undefined || kind === cdef('ClauseRef')) return;
    }
  }
  throw new ConceptHashError(
    ERR.GRAMMAR,
    'TRUE undergoer must be a ClauseRef referent or a quote — no "true-of" predication exists (§4.4)',
  );
}

function isPrimeHeaded(ctx: Ctx, filler: Term, prime: string): boolean {
  if (filler.termType === 'NamedNode') return primeName(ctx, filler) === prime;
  if (filler.termType !== 'BlankNode') return false;
...
function checkFiller(ctx: Ctx, filler: Term, scope: Scope, fctx: FillerCtx): void {
  const { ix } = ctx;
  if (filler.termType === 'NamedNode') {
    ...
    if (name !== undefined && !FILLER_PRIMES.has(name)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `prime ${name} is not a licensed direct filler (§4.5 indexicals/durations)`,
      );
    }
  ...
  if (preds.has(QUOTE_CLAUSES)) {
    if (fctx.role !== 'quote' && fctx.role !== 'complement' && fctx.role !== 'undergoer') {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `a quote filler is not licensed in role ${fctx.role} (§4.4)`,
      );
    }
    const quoteScope: Scope = { introduced: new Set(), parent: scope };
...
function checkSubstantivePhrase(ctx: Ctx, node: Term, scope: Scope): void {
  const { ix } = ctx;
  ...
  if (ref !== undefined) {
    ...
    if (!inScope(scope, idx)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `cdef:ref ${idx} references an undeclared or not-yet-introduced referent (§4.2)`,
      );
    }
  }
  ...
  if (det !== undefined) {
    ...
    if (name === undefined || !DET_PRIMES.has(name)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `cdef:det ${describe(det)} is not a determiner prime (§4.3)`,
      );
    }
  }
  ...
  if (quant !== undefined) {
    ...
    if (name === undefined || !QUANT_PRIMES.has(name)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `cdef:quant ${describe(quant)} is not a quantifier prime (§4.3)`,
      );
    }
  }
  for (const m of ix.objects(node, MOD)) {
    ...
    if (name === undefined || !MOD_PRIMES.has(name)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `cdef:mod ${describe(m)} is not a modifier prime (§4.3)`,
      );
    }
  }
  ...
  if (head !== undefined) {
    ...
    if (name !== undefined && !SUBSTANTIVE_PRIMES.has(name)) {
      throw new ConceptHashError(
        ERR.GRAMMAR,
        `prime ${name} cannot head a substantive phrase (§4.3)`,
      );
    }
    if (of !== undefined) {
      if (name !== 'KIND' && name !== 'PART') {
        throw new ConceptHashError(
          ERR.GRAMMAR,
          'cdef:of is licensed only on KIND/PART-frame heads (§4.3)',
        );
      }
