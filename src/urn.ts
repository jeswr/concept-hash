...
export function digestToUrn(raw: Uint8Array, code: number = SHA2_256_CODE): string {
...
export function urnToMultibase(urn: string): string {
  parseConceptUrn(urn);
 ...
 */
export function parseConceptUrn(urn: string): ParsedUrn {
  ...
  try {
    ...
    throw new ConceptHashError(
      ...
      `multibase decode failed for ${urn}: ${String(cause)}`,
    );
  }
  let mh: ReturnType<typeof Digest.decode>;
  try {
    mh = Digest.decode(bytes);
  } catch (cause) {
    throw new ConceptHashError(
      ERR.REFERENCE_SYNTAX,
      `multihash decode failed for ${urn}: ${String(cause)}`,
    );
  }
...
export function isConceptUrn(s: string): boolean {
  try {
    parseConceptUrn(s);
    return true;
  } catch {
