 ...
 */
export function checkGroundingNote(raw) {
    const note = raw.normalize('NFC');
    const bytes = Buffer.byteLength(note, 'utf8');
    if (bytes > CAPS.maxGroundingNoteBytes) {
        throw new ConceptHashError(ERR.CAPS, `grounding note is ${bytes} bytes NFC (cap ${CAPS.maxGroundingNoteBytes}, §3.5)`);
    }
    if (note.includes('\u0000')) {
    ...
    for (const m of note.matchAll(LINKED_REF_RE)) {
        ...
        try {
            ...
            throw new ConceptHashError(ERR.GROUNDING_LEXICON, `grounding-note linked ref ${urn} is not a valid final urn:concept: URN (§3.5 rule 4): ${cause instanceof Error ? cause.message : String(cause)}`);
        }
        refs.add(urn);
    ...
    if (stripped.includes('{') || stripped.includes('}') || stripped.includes('|')) {
        throw new ConceptHashError(ERR.GROUNDING_LEXICON, 'malformed linked reference in grounding note — the only legal brace form is {urn:concept:…|gloss}');
    }
    for (const rawToken of stripped.split(/\s+/)) {
        if (rawToken === '')
            continue;
        if (rawToken === MOLECULE_FLAG_TOKEN)
            continue;
        // strip closed punctuation from token edges (e.g. "kind;" / "(very")
        let token = rawToken;
        while (token.length > 0) {
            const first = token[0];
            if (first !== undefined && GROUNDING_PUNCTUATION.has(first))
                token = token.slice(1);
            else
                break;
        }
        while (token.length > 0) {
            ...
            if (lastCh !== undefined && GROUNDING_PUNCTUATION.has(lastCh))
        ...
        if (!GROUNDING_LEXICON.has(token.toLowerCase())) {
            throw new ConceptHashError(ERR.GROUNDING_LEXICON, `grounding-note token ${JSON.stringify(token)} is not a prime exponent, closed allolex, punctuation, [m], or linked ref (§3.5 rule 3)`);
        }
    }
    return refs;
}
