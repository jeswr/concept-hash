...
export const namedNode = (value) => ({ termType: 'NamedNode', value });
export const blankNode = (value) => ({ termType: 'BlankNode', value });
export const literal = (value, datatype = XSD_STRING, language = '') => ({
    termType: 'Literal',
    value,
    datatype,
    language,
});
export const defaultGraph = () => ({ termType: 'DefaultGraph', value: '' });
...
export function toCanonizeQuad(q) {
    return {
 ...
 */
export function normalizeLiteral(t) {
    ...
    switch (datatype) {
        ...
        case XSD_NON_NEG: {
            if (!/^[+-]?\d+$/.test(value)) {
                throw new ConceptHashError(ERR.SHAPE, `invalid ${datatype} literal: ${JSON.stringify(value)}`);
            }
        ...
        case XSD_DECIMAL: {
            if (!/^[+-]?(\d+(\.\d*)?|\.\d+)$/.test(value)) {
                throw new ConceptHashError(ERR.SHAPE, `invalid xsd:decimal literal: ${JSON.stringify(value)}`);
            }
            const neg = value.startsWith('-');
        ...
        case XSD_DOUBLE: {
            if (!/^[+-]?(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?$/.test(value) &&
                !/^(\+|-)?INF$|^NaN$/.test(value)) {
                throw new ConceptHashError(ERR.SHAPE, `invalid xsd:double literal: ${JSON.stringify(value)}`);
            }
        ...
        case XSD_BOOLEAN: {
            if (value === '1')
                ...
                throw new ConceptHashError(ERR.SHAPE, `invalid xsd:boolean literal: ${JSON.stringify(value)}`);
            }
            break;
        }
        default:
...
export function isLiteralCanonical(t) {
...
function normalizeTerm(t) {
    ...
    if (t.termType === 'NamedNode')
        return namedNode(t.value.normalize('NFC'));
    return t;
}
/** Normalize every literal (and NFC every IRI) in a quad set. */
export function normalizeQuads(quads) {
    return quads.map((q) => ({
 ...
 */
export async function canonicalNQuads(quads) {
 ...
 */
export function firstDegreeHashes(quads) {
    const byBnode = new Map();
    ...
    for (const [bnode, mentions] of byBnode) {
        const lines = mentions.map((q) => {
            const sub = (t) => t.termType === 'BlankNode' ? blankNode(t.value === bnode ? 'a' : 'z') : t;
            return NQuads.serializeQuad(toCanonizeQuad({
 ...
 */
export function assertNoDuplicateFdh(quads) {
    let colors = firstDegreeHashes(quads);
    if (colorsUnique(colors))
        return;
    ...
    for (let round = 0; round < REFINEMENT_ROUNDS; round++) {
        ...
        if (nextDistinct === distinct)
            break; // partition stabilized — automorphic remainder
    ...
    for (const [bnode, color] of colors) {
        ...
        if (prior !== undefined) {
            throw new ConceptHashError(ERR.DUPLICATE_BNODE_FDH, `blank nodes _:${prior} and _:${bnode} are indistinguishable after color refinement (duplicate first-degree hash class; identical sibling subtrees are redundant assertions)`);
        }
        seen.set(color, bnode);
    }
}
function colorsUnique(colors) {
