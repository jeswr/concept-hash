 ...
 */
import { termEquals } from './quads.js';
import { AXIOM, CAPS, CHART_EDITION, CHART_INDEX, ConceptHashError, ERR, EXPLICATION, EXPONENT, GROUNDING_NOTE, PRIME_CATEGORY, RDF_FIRST, RDF_NIL, RDF_REST, RDF_TYPE, SEMANTIC_STATUS, } from './vocab.js';
/** Predicates the extractor follows from the focus subject (§5). */
export const FOCUS_PREDICATES = new Set([
    RDF_TYPE,
    SEMANTIC_STATUS,
    AXIOM,
    EXPLICATION,
 ...
 */
export function extractRecord(quads, focus, strict = false) {
    ...
    const focusQuads = (bySubject.get(`I${focus}`) ?? []).filter((q) => FOCUS_PREDICATES.has(q.predicate.value));
    if (focusQuads.length === 0) {
        throw new ConceptHashError(ERR.SHAPE, `no profile-vocabulary triples found for focus <${focus}>`);
    }
    const visit = (bnode, depth, viaList) => {
        if (seenBnodes.has(bnode)) {
            throw new ConceptHashError(ERR.SHAPE, `blank node _:${bnode} has more than one parent or participates in a cycle — records are bnode TREES (§5)`);
        }
        seenBnodes.add(bnode);
        bnodes.push(bnode);
        if (bnodes.length > CAPS.maxTotalBnodes) {
            throw new ConceptHashError(ERR.CAPS, `record exceeds ${CAPS.maxTotalBnodes} total blank nodes (cap, §5)`);
        }
        const own = bySubject.get(`B${bnode}`) ?? [];
        const isListCell = own.some((q) => q.predicate.value === RDF_FIRST || q.predicate.value === RDF_REST);
        if (isListCell)
            listCells.add(bnode);
        ...
        if (childDepth > CAPS.maxNestingDepth) {
            throw new ConceptHashError(ERR.CAPS, `record exceeds structural nesting depth ${CAPS.maxNestingDepth} (cap, §5)`);
        }
    ...
    if (strict) {
        ...
        if (quads.length !== record.length) {
            throw new ConceptHashError(ERR.VERIFY, `served representation contains ${quads.length - record.length} triple(s) outside the definition record (§5 verify rule: REJECT, not ignore)`);
        }
    }
...
export class RecordIndex {
    ...
    one(subject, predicate) {
        ...
        if (values.length !== 1) {
            throw new ConceptHashError(ERR.SHAPE, `expected exactly one <${predicate}> on ${describe(subject)}, found ${values.length}`);
        }
    ...
    optional(subject, predicate) {
        const values = this.objects(subject, predicate);
        if (values.length > 1) {
            throw new ConceptHashError(ERR.SHAPE, `expected at most one <${predicate}> on ${describe(subject)}, found ${values.length}`);
        }
        return values[0];
    }
    ...
    readList(head) {
        ...
        let cursor = head;
        while (true) {
            if (cursor.termType === 'NamedNode' && cursor.value === RDF_NIL)
                return items;
            ...
            if (firsts.length !== 1 || rests.length !== 1 || cell.length !== 2) {
                throw new ConceptHashError(ERR.SHAPE, `rdf:List cell _:${cursor.value} must have exactly rdf:first + rdf:rest`);
            }
            ...
            if (items.length > CAPS.maxListLength) {
                throw new ConceptHashError(ERR.CAPS, `rdf:List exceeds ${CAPS.maxListLength} items (cap, §5)`);
            }
            cursor = rest.object;
        }
    }
...
export function namedNodesIn(quads) {
    const out = new Set();
    for (const q of quads) {
...
export function substituteIris(quads, mapping) {
    const sub = (t) => {
        if (t.termType === 'NamedNode') {
            const to = mapping.get(t.value);
            if (to !== undefined)
                return { termType: 'NamedNode', value: to };
        }
        return t;
    };
    return quads.map((q) => ({
