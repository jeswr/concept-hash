...
async function loadAliases(path) {
    ...
    if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        throw new ConceptHashError('ERR_SHAPE', 'alias table must be a JSON object of name → urn:concept: URN');
    }
    const out = {};
        ...
        switch (command) {
            case 'hash': {
                ...
                if (json) {
                    ...
                    for (const [name, r] of result.records) {
                        const extra = r.component === undefined
                            ? ''
            ...
            case 'verify': {
                ...
                const result = await verifyServed(urn, quads, format === 'application/n-quads' ? { servedNQuads: text } : {});
                if (json) {
                    process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
                }
                else {
                    const detail = result.kind === 'component-member'
