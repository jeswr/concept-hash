...
import { assertNoDuplicateFdh, canonicalNQuads, isLiteralCanonical } from './quads.js';
...
import { ALL_PROFILE_TERMS, CAPS, ConceptHashError, ERR, INTRA, MEMBER_HEADER, MINT_SCHEME, memberIri, PROFILE_HEADER, SELF, } from './vocab.js';
 ...
 */
export async function verifyServed(urn, quads, options = {}) {
    ...
    for (const q of quads) {
        if (q.graph.termType === 'DefaultGraph')
            defaultGraphQuads += 1;
        else if (q.graph.termType === 'NamedNode')
            namedGraphs.add(q.graph.value);
        else
            throw new ConceptHashError(ERR.VERIFY, 'blank-node graph names are not record-shaped');
    }
    if (namedGraphs.size === 0) {
        return verifySingleton(urn, quads, options);
    }
    if (defaultGraphQuads > 0) {
        throw new ConceptHashError(ERR.VERIFY, 'a component dataset must have an EMPTY default graph (§6 step 8)');
    }
    return verifyComponent(urn, quads, namedGraphs, options);
}
// ---- singleton (§7) --------------------------------------------------------------
async function verifySingleton(urn, quads, options) {
    ...
    if (computed !== urn) {
        throw new ConceptHashError(ERR.VERIFY, `recomputed URN ${computed} does not match requested ${urn}`);
    }
    return { ok: true, kind: 'singleton', urn };
}
// ---- component (§7) ---------------------------------------------------------------
async function verifyComponent(urn, quads, namedGraphs, options) {
    const n = namedGraphs.size;
    if (n < 2 || n > CAPS.maxSccSize) {
        throw new ConceptHashError(ERR.VERIFY, `component must have 2..${CAPS.maxSccSize} member graphs, found ${n}`);
    }
    for (let i = 0; i < n; i++) {
        if (!namedGraphs.has(memberIri(i))) {
            throw new ConceptHashError(ERR.VERIFY, `member graphs must be named exactly #member-0…#member-${n - 1} (contiguous); missing #member-${i}`);
        }
    }
    const memberFoci = new Set();
    for (let i = 0; i < n; i++)
        memberFoci.add(memberIri(i));
    ...
    for (let i = 0; i < n; i++) {
        ...
        const inGraph = quads
            ...
            .map((q) => ({ ...q, graph: { termType: 'DefaultGraph', value: '' } }));
    ...
    for (let i = 0; i < n; i++) {
        const mapping = new Map();
        for (let j = 0; j < n; j++)
            mapping.set(memberIri(j), j === i ? SELF : INTRA);
    ...
    for (let i = 1; i < keys.length; i++) {
        ...
        if (a !== undefined && b !== undefined && !(a < b)) {
            throw new ConceptHashError(ERR.VERIFY, `member indices do not follow the canonical ordering-key sort (§6 steps 6–7): key[${i - 1}] ≥ key[${i}]`);
        }
    }
    // steps 8–9 recompute
    ...
    const memberUrns = [];
    for (let i = 0; i < n; i++) {
        memberUrns.push(digestToUrn(sha256(concatBytes(utf8(MEMBER_HEADER), componentDigest, uvarint(i)))));
    }
    if (urn === componentUrn) {
    ...
    if (memberIndex === -1) {
        throw new ConceptHashError(ERR.VERIFY, `requested URN ${urn} is neither the component URN nor any of its ${n} member URNs`);
    }
...
function assertServedTermsCanonical(quads) {
    for (const q of quads) {
        for (const t of [q.subject, q.predicate, q.object, q.graph]) {
            if (t.termType === 'NamedNode' && t.value !== t.value.normalize('NFC')) {
                throw new ConceptHashError(ERR.NOT_CANONICAL, `served IRI <${t.value}> is not NFC (§7: reject, never normalize)`);
            }
            if (t.termType === 'Literal') {
                try {
                    if (!isLiteralCanonical(t)) {
                        throw new ConceptHashError(ERR.NOT_CANONICAL, `served literal ${JSON.stringify(t.value)} is not in canonical form (§7: reject, never normalize)`);
                    }
                }
                catch (cause) {
                    if (cause instanceof ConceptHashError && cause.code === ERR.NOT_CANONICAL)
                        throw cause;
                    throw new ConceptHashError(ERR.NOT_CANONICAL, `served literal ${JSON.stringify(t.value)} has an invalid lexical form`);
                }
            }
        }
    }
 ...
 */
function assertComponentReferenceDiscipline(quads, memberFoci) {
    for (const q of quads) {
        for (const t of [q.subject, q.predicate, q.object]) {
            ...
            if (v.startsWith(MINT_SCHEME)) {
                throw new ConceptHashError(ERR.VERIFY, `served content contains a symbolic reference <${v}>`);
            }
            if (v.startsWith('urn:concept-def:')) {
                if (memberFoci.has(v))
                    continue;
                if (ALL_PROFILE_TERMS.has(v))
                    continue;
                throw new ConceptHashError(ERR.RESERVED_TOKEN, `<${v}> is not a legal token inside a component dataset (§5 reservation; #self/#intra never appear in step-8 output)`);
            }
...
function assertByteIdentical(recomputed, options) {
    if (options.servedNQuads !== undefined && options.servedNQuads !== recomputed) {
        throw new ConceptHashError(ERR.NOT_CANONICAL, 'served application/n-quads bytes are not the canonical bytes for this record (§7: exactly one byte representation exists per URN)');
    }
}
function assertCanonicalSize(nquads) {
    const bytes = Buffer.byteLength(nquads, 'utf8');
    if (bytes > CAPS.maxCanonicalBytes) {
        throw new ConceptHashError(ERR.CAPS, `canonical record size ${bytes} exceeds ${CAPS.maxCanonicalBytes} (cap, §5)`);
    }
}
