...
export const memberIri = (i) => `${CDEF}member-${i}`;
...
export const cdef = (local) => `${CDEF}${local}`;
// --- record-level terms ------------------------------------------------------
...
export const REF_KINDS = new Set(['SomeoneRef', 'SomethingRef', 'TimeRef', 'PlaceRef', 'ClauseRef'].map(cdef));
...
export const servingPath = (multibase, host = DEFAULT_SERVE_HOST) => `https://${host}/i/${multibase}`;
 ...
 */
export const ALL_PROFILE_TERMS = new Set([
    CONCEPT_DEFINITION,
    SEMANTIC_STATUS,
    ...SEMANTIC_STATUSES,
    AXIOM,
