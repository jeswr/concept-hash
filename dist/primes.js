...
const p = (name, category, chartIndex, exponents) => ({
    name,
    category,
    chartIndex,
...
export const PRIME_BY_NAME = new Map(PRIMES.map((e) => [e.name, e]));
export const PRIME_BY_INDEX = new Map(PRIMES.map((e) => [e.chartIndex, e]));
/**
 ...
 */
const PRIME_SURFACE_FORMS = PRIMES.flatMap((e) => e.exponents).map((x) => x.toLowerCase());
...
export const GROUNDING_LEXICON = new Set([
    ...PRIME_SURFACE_FORMS,
    ...INFLECTED_FORMS,
    ...FUNCTION_WORDS,
]);
/** Closed punctuation set for grounding notes (matched per-character). */
export const GROUNDING_PUNCTUATION = new Set([
    '.',
    ',',
    ';',
    ':',
